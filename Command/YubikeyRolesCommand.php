...
 
class YubikeyRolesCommand extends ContainerAwareCommand {
 
...
 
    protected function configure() {
        ...
        $this
                ->setName('yubikey_login:roles')
                ->setDescription('Get the user roles given the yubikey OTP')
                ->addArgument(
...
 
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        foreach ($yubikey_user->getRoles() as $role) {
            $output->writeln($role);
        }
    }
 
